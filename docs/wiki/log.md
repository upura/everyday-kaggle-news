...
- 2026-07-16 lint: GitHub Issue #3（リンク切れ検査 2026-07、116件の「リンク切れ」+134件の「要手動確認」）を全件 WebFetch で再検証し、対応した。(1) 根本原因として `tools/check_external_links.py` の非ブラウザ HTTP クライアントが `kaggle.com`（35件）・`medium.com`（47件）をボット対策で誤って 404/403 扱いしていたことが判明。両ドメインを `MANUAL_DOMAINS` に追加し、以後は「要手動確認」に分類されるよう修正（`.claude/skills/lint/SKILL.md` の説明も更新）。amzn.asia は元々正しく分類されていた。(2) 残り約209件を1件ずつ WebFetch で検証し、確実に404/410/ドメイン消滅と確認できたもの（約100件強）のみ資料一覧から削除。solutions.md では削除の結果、資料が0件になった div が3件（LLMs - You Can't Please Them All／Vesuvius Challenge - Ink Detection／CommonLit Readability Prize）あったため div ごと削除。(3) 生存を確認できたがURLが変わっていたもの2件はリダイレクト先に更新（PyTorch3D: ai.facebook.com→ai.meta.com、DataRobotリーク記事: blog.datarobot.com→www.datarobot.com）、証明書のホスト名不一致だった3件（mattari-benkyo-note.com）は www. を外して修正。(4) SSL/証明書エラー・403・タイムアウトなど生死を確定できなかった約38件は「明確に死んでいる場合のみ削除する」方針に従い変更せず保持（follow-up として残る）。作業は `fix/broken-links-2026-07` ブランチで実施し、PR でレビュー後にマージする
