...
- 2026-09-05 ingest: WKN #351 → 新規 8 件（solutions.md の ROGII div に 1 件 / solutions.md に div 2 新設（AI Agent Security - Multi-Step Tool Attacks に 3 件・The Pokémon Company - PTCG AI Battle Challenge Simulation に 1 件）/ events.md の「その他」に解法共有会 1 件 / books.md に 1 件 / concepts/nlp-llm-trends に第 18 回最先端 NLP 勉強会 / concepts/competition-evolution・concepts/agent-competition に AI Agent Security のコンペページ）。competition-evolution の「押さえどころ」に 1 項目（エージェントを攻撃対象とする形式の登場と、Public/Private の評価差に対して失敗理由の異なる提出を用意する戦略）、ai-agent に 2 項目（方針決定と採否判定は人・実装と確認はエージェントという分担、および判定基準の設計と自己チェックの限界）、agent-competition に 3 項目（評価条件を固定しないと改善を見誤る点、ルールベースから探索・価値関数・行動クローン・自己対戦・相手推定へ積み上げる定番の順序、エージェントを攻撃対象に据える形式の登場）を追記。The Pokémon Company - PTCG AI Battle Challenge Simulation の最終結果判明と AI Agent Security の終了の報は時限性のためスキップ（両コンペのページは concepts/agent-competition・competition-evolution に既掲載、または今回追加）。コンペ URL は末尾スラッシュなしに揃えた（docs/competitions.json の表記と一致）。Substack へは GitHub Actions 環境から直接アクセスできず（Cloudflare のボット対策）、テキスト抽出プロキシ経由で号とアーカイブを取得した。なお WKN #350（2026-08-28 配信）は本ログに記録がなく未取り込みだった（今回の対象は最新号のため未着手。2026-09-11 の行で遡って取り込み済み）
- 2026-09-11 ingest: WKN #344（2026-07-17 配信。weekly-ingest が固定ブランチを上書きしていたため未取り込みだった分の遡及）→ 新規 5 件（events.md に Fields Model Day（AIMO 報告会）/ concepts/competition-hosting に Kaggle コンペ開催費用の記事 / concepts/ai-agent に「コーディングエージェント時代のKaggle」 / concepts/mindset に「Kaggleで成長するために意識したこと」 / concepts/competition-evolution に The 2026 NeuroGolf Championship のコンペページ）。ai-agent の「押さえどころ」に 1 項目（実装が容易になった分、勝負どころが勝敗の決まり方の見極め・CV 設計・データ解釈へ移る点）、mindset に 1 項目（目的が順位の最大化にすり替わることで停滞が起きる構図）を追記
- 2026-09-11 ingest: WKN #345（2026-07-24 配信。同上の遡及）→ 新規 5 件（concepts/nlp-llm-trends に Gemini 3.6 Flash ほかの追加発表と HAKARI-Bench / service.md に Kaggle AI Benchmarks / concepts/competition-hosting に「Measuring Progress Toward AGI」ハッカソンの受賞発表スレッド / concepts/competition-evolution に Kaggle Head of Product のインタビュー動画）。competition-hosting の「資料」に小見出し「審査・運営ルールの設計」を新設した
- 2026-09-11 ingest: WKN #346（2026-07-31 配信。同上の遡及）→ 新規 5 件（concepts/agent-competition と concepts/competition-evolution に Kaggriculture のコンペページ / concepts/competition-hosting にポケカコンペの公開ノートブック締切の告知 / concepts/nlp-llm-trends に Bekko Embedding / events.md に Kaggle x コーディングエージェント勉強会の録画と資料まとめ）。competition-evolution の「押さえどころ」に 1 項目（対戦・攻撃・モデルの小ささなど、競う対象そのものが多様化した点）を追記。3 号とも Substack の JSON API（/api/v1/posts/<slug>）から本文を取得し、コンペの概要と規模は Kaggle の内部 API（GetCompetition）で確認した
- 2026-09-11 ingest: WKN #350（2026-08-28 配信。#351 の取り込み時に飛ばしていた分を遡って対応）→ 新規 5 件（service.md に AIYSS Kaggle League / concepts/environment に P100 提供終了の公式アナウンス / concepts/ai-agent に KDD Cup 2026「Data Agents for Complex Data Analysis」の公式サイトと Presentation Archive / concepts/mindset に「Kaggler流のプロダクト開発入門！」）。environment の「押さえどころ」に 1 項目（アクセラレータの世代交代と T4x2 への一本化）、ai-agent に 1 項目（データ分析エージェント自体を競わせるコンペの登場と、単一スコアで測りにくい対象の評価）、mindset に 1 項目（コンペの 4 段階をプロダクト開発に対応づける整理と、最小構成を先に通す勘所）を追記。The Pokémon Company - PTCG AI Battle Challenge Simulation の振り返り会（10 月 15 日）は号の時点で詳細未公開のためスキップ。号の本文は Substack の JSON API（/api/v1/posts/<slug>）から取得した。dataagent.top と aiyss.org は JS 描画のため HTML からは本文を取得できず、配信されている JS バンドルと構造化データから内容を確認している
- 2026-08-22 ingest: WKN #349 → 新規 5 件（milestones.md の「称号分析」に 1 件 / concepts/competition-hosting に 4 件）。competition-hosting の「押さえどころ」に 3 項目（金融予測コンペの設計史、CZII の後継コンペ検討で分野側と参加者側の望む設計が食い違った件、学会の開催支援制度の広報面での効果）、competition-evolution の「押さえどころ」に 1 項目（upload 形式の縮小がメダルの情報価値を下げたという分析）を追記。「The Pokémon Company - PTCG AI Battle Challenge Simulation」の提出締切の報は時限性のためスキップ（コンペ自体は concepts/agent-competition と concepts/competition-evolution に既掲載）。CZII のコンペページは solutions.md に既掲載。人工知能学会の開催支援制度は旧 URL（www.ai-gakkai.or.jp 側）の募集案内が既掲載だが、今回の kikaku.ai-gakkai.or.jp 側は 2026 年度の条件（1 件あたり上限 50 万円）と過去の採択コンペ一覧を含む別ページのため、重複扱いせず併記した。Substack へは GitHub Actions 環境から直接アクセスできず（Cloudflare のボット対策）、テキスト抽出プロキシ経由で号を取得した。Nature Methods の CZII 論文も同様に認証リダイレクトで WebFetch が通らず、プロキシ経由で本文を確認している
- 2026-08-15 ingest: WKN #348 → 新規 5 件（solutions.md の ROGII - Wellbore Geology Prediction div に 2 件 / events.md の第 6 回関東 Kaggler 会に参加録 2 件 / concepts/tabular に XGBoost 原論文）。tabular の「押さえどころ」に XGBoost 原論文を起点とする GBDT の系譜を、time-series の「押さえどころ」に系列を 2 次元画像化して U-Net で解く定跡を追記。Kaggle「Head of AI Ecosystem」の求人情報は時限性のためスキップ。NVIDIA Nemotron Model Reasoning Challenge の優勝チーム配信は 8 月 26 日の配信予告で内容を確認できないためスキップ（コンペの div は既存）。Substack へは GitHub Actions 環境から直接アクセスできず（Cloudflare のボット対策）、テキスト抽出プロキシ経由で号を取得した。XGBoost 論文は ACM DL が bot 対策で取得できないため、arXiv 版（1603.02754）で内容を確認した上で号の記載どおり ACM DL の DOI URL を掲載している
- 2026-08-08 ingest: WKN #347 → 新規 6 件（books.md / solutions.md に div 1 新設（ROGII - Wellbore Geology Prediction）/ concepts/image-recognition, competition-hosting ×2, competition-evolution）。competition-hosting と competition-evolution の「押さえどころ」に各 1 項目を追記。「第 6 回関東 Kaggler 会」は events.md に既掲載のためイベント自体はスキップし、発表資料のみ competition-evolution に掲載。RSNA Knee Abnormality Detection の開催告知と ROGII の終了告知は時限性のためスキップ。Substack へは GitHub Actions 環境から直接アクセスできず（Cloudflare のボット対策）、テキスト抽出プロキシ経由で号を取得した。書籍『マルチモーダルモデル入門』は Amazon のページが 500 を返したため、号の記載（技術評論社・2026 年 9 月刊）に基づいて登録している
- 2026-07-31 restructure: 「[コンペ形式・技術動向の変遷](./concepts/competition-evolution.md)」のデータ種別図に 2026 年（進行中）を暫定値として追加（2015〜2025 → 2015〜2026）。2026 はコンペタグが未整備のため、メダル対象 24 件のうち内容が明確な 18 件を締切・タイトルから手作業で分類（データ種別に馴染まない 6 件＝Santa 最適化・RNA 3D Folding・ARC-AGI-2／3・NeuroGolf・Kaggriculture は除外）。他の年（タグ由来）と集計方法が異なる旨、および ARC・AIMO 等の推論・エージェント系が「データ種別」に収まらず 2026 のシフトを過小評価しうる旨を図の注記に明記。2026 の棒は淡色＋斜体の `'26*` ラベルで暫定と区別
- 2026-07-31 restructure: 管理者の依頼により概念ページ「[コンペ形式・技術動向の変遷](./concepts/competition-evolution.md)」を全体更新。Kaggle 公式 Meta Kaggle データセットから抽出したメダル対象コンペ（`CanQualifyTiers` が真の 461 件）を締切年で集計し、データ種別の年次構成（2015〜2025）を「構成比／件数」切り替え・ホバー付きのインタラクティブな積み上げ棒グラフとして新セクション「データで見る：メダル対象コンペのデータ種別構成」に埋め込み（`docs/calendar.md` と同じインライン SVG 方式。テンプレートは維持しつつ図を追加）。あわせて「押さえどころ」に定量的な裏づけ（表データの構成比が 2015 年の 8 割超から 2020 年代半ばに 1〜2 割へ低下、2019 年は画像が過半の 27 件中 14 件、2024 年はテキストが単年最大種別の 8 件など）を追記。図データは Meta Kaggle の 2026-07-31 取得時点のスナップショットを JS 内にハードコードしており、データ種別はコンペタグからの推定（タグ付き 245/326 件、カバー率 75%）である旨を図に注記。資料エントリは変更していない
- 2026-07-17 restructure: 管理者の依頼により一覧ページ「[コンペ開催カレンダー](../calendar.md)」を新設。Pages ビルド（jekyll-gh-pages ワークフロー、push 時に加えて毎日 06:00 JST に定期実行、`tools/fetch_competitions.py`）が Kaggle API から開催中のメダル対象コンペ（awardsPoints が真のもの）を取得して `docs/competitions.json` を生成し、`docs/calendar.md` がカテゴリ別の色分け・絞り込み・ツールチップ・表形式ビュー付きのタイムラインで表示する。締切が 400 日以上先のコンペは「常設・長期開催」として分離して表示。README.md の目次（「情報収集・コミュニティ」）と [最新情報の確認](../recent.md) の「コンペの開催情報」に導線を追加。稼働にはリポジトリシークレット KAGGLE_API_TOKEN（従来形式の KAGGLE_USERNAME / KAGGLE_KEY でも可）の設定が必要
