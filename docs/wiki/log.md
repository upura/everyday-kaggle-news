...
- 2026-09-05 ingest: WKN #351 → 新規 8 件（solutions.md の ROGII div に 1 件 / solutions.md に div 2 新設（AI Agent Security - Multi-Step Tool Attacks に 3 件・The Pokémon Company - PTCG AI Battle Challenge Simulation に 1 件）/ events.md の「その他」に解法共有会 1 件 / books.md に 1 件 / concepts/nlp-llm-trends に第 18 回最先端 NLP 勉強会 / concepts/competition-evolution・concepts/agent-competition に AI Agent Security のコンペページ）。competition-evolution の「押さえどころ」に 1 項目（エージェントを攻撃対象とする形式の登場と、Public/Private の評価差に対して失敗理由の異なる提出を用意する戦略）、ai-agent に 2 項目（方針決定と採否判定は人・実装と確認はエージェントという分担、および判定基準の設計と自己チェックの限界）、agent-competition に 3 項目（評価条件を固定しないと改善を見誤る点、ルールベースから探索・価値関数・行動クローン・自己対戦・相手推定へ積み上げる定番の順序、エージェントを攻撃対象に据える形式の登場）を追記。The Pokémon Company - PTCG AI Battle Challenge Simulation の最終結果判明と AI Agent Security の終了の報は時限性のためスキップ（両コンペのページは concepts/agent-competition・competition-evolution に既掲載、または今回追加）。コンペ URL は末尾スラッシュなしに揃えた（docs/competitions.json の表記と一致）。Substack へは GitHub Actions 環境から直接アクセスできず（Cloudflare のボット対策）、テキスト抽出プロキシ経由で号とアーカイブを取得した。なお WKN #350（2026-08-28 配信）は本ログに記録がなく未取り込みの可能性がある（今回の対象は最新号のため未着手）
