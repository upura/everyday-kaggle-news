...
基礎はチュートリアルで押さえ、学習・推論の高速化テクニックで実験の回転を上げます。

## 押さえどころ

- 基礎は公式チュートリアル(日本語翻訳版あり)を一巡するのが近道
- 学習・推論の高速化(DataLoader 設定・AMP など)は、実験回数に直結するため早めに習慣化する
- 事前学習済みモデルの活用は [画像認識コンペ](./image-recognition.md)、Transformer 系の fine-tuning は [自然言語処理・LLM コンペ](./nlp-llm.md) を参照
...
- [画像認識コンペ](./image-recognition.md) / [自然言語処理・LLM コンペ](./nlp-llm.md) / [環境構築](./environment.md)
