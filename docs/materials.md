## 目次

本ページは、Everyday Kaggle News 全体の目次です。
話題ごとに、Wiki ページ(概説・押さえどころ・資料一覧)と一覧ページを案内します。
リンクの横断検索は「[検索](./search.md)」から。

## 最近の更新

直近の更新の抜粋です(全履歴は[操作ログ](./wiki/log.md))。

- 2026-07-11: サイト内検索と共通ナビゲーションを追加
- 2026-07-11: Weekly Kaggle News #309〜#324 を反映
- 2026-07-11: 概念ページ「[エージェント対戦コンペ](./wiki/concepts/agent-competition.md)」を新設
- 2026-07-10: 週次の自動取り込み(GitHub Actions + PR レビュー)を開始
- 2026-07-10: Weekly Kaggle News #325〜#343 を反映

## 学び方

- [入門者・初学者向けガイド](./quickstart.md): Kaggle の概要と始め方、まず読む・見る定番資料
- [心構え](./wiki/concepts/mindset.md): コンペとの向き合い方・学び方・実務との関係
- [書籍](./books.md): Kaggle 関連書籍の一覧
- [称号振り返り・インタビュー](./milestones.md): 称号到達の振り返り記事とインタビュー

## 情報収集・コミュニティ

- [最新情報の確認](./recent.md): 公式・有志による情報源
- [コンペプラットフォーム](./platform.md): Kaggle 以外のコンペプラットフォームとコンペの選び方
- [イベント](./events.md): 勉強会・ミートアップなどの記録
- [サービス・ツール](./service.md): 非公式のサービス・ツール

## データ種別・タスク

- [コンペ解法](./solutions.md): コンペ別の解法・参加録(年・データ種別・プラットフォームで絞り込み可)
- [表データコンペ](./wiki/concepts/tabular.md): GBDT・特徴量エンジニアリング・Polars・表向け NN
- [画像認識コンペ](./wiki/concepts/image-recognition.md): バックボーン選択・公開モデルの活用と高速化
- [自然言語処理コンペ](./wiki/concepts/nlp-llm.md): BERT 系の定跡と LLM 活用・推論高速化
- [時系列予測コンペ](./wiki/concepts/time-series.md): 時間分割バリデーション・損失設計・時系列基盤モデル
- [音声コンペ](./wiki/concepts/audio.md): メルスペクトログラム化と画像モデル適用・異常音検知
- [グラフ](./wiki/concepts/graph.md): グラフ構造データの学習
- [推薦](./wiki/concepts/recommendation.md): 推薦アルゴリズムの基礎と 2 段構成の解法
- [数理最適化コンペ](./wiki/concepts/optimization.md): Santa 系コンペ・AtCoder との親和性
- [エージェント対戦コンペ](./wiki/concepts/agent-competition.md): エージェント提出・対戦形式のコンペと Game Arena などの評価基盤

## 進め方・環境

- [PyTorch](./wiki/concepts/pytorch.md): 深層学習実装の基礎と高速化
- [コードコンペティション](./wiki/concepts/code-competition.md): 提出形式の制約と作業フロー・提出自動化
- [実験管理](./wiki/concepts/experiment-management.md): 試行回数の最大化・再現性・チーム体制
- [環境構築](./wiki/concepts/environment.md): ローカル・クラウドの計算環境
- [性能評価と検証](./wiki/concepts/evaluation-validation.md): リーク防止・バリデーション設計・データセット品質
- [AI エージェント活用](./wiki/concepts/ai-agent.md): LLM エージェントによるコンペ作業の自動化と限界

## コンペの開催

- [学会コンペ](./wiki/concepts/academic-competition.md): 学会・研究コミュニティが主催するコンペ
- [コンペ開催](./wiki/concepts/competition-hosting.md): コンペを開く側の知見・開催報告

## 運用

各話題ページは、複数の資料を横断した知見を LLM が [CLAUDE.md](https://github.com/upura/everyday-kaggle-news/blob/main/CLAUDE.md) の規約に従って維持しています。
更新履歴は[操作ログ](./wiki/log.md)を参照してください。
